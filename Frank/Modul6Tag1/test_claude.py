"""pytest-Suite fuer `check_encoding_v2.py`.
...
_MODULE_PATH = Path(__file__).with_name("check_encoding_v2.py")
_spec = importlib.util.spec_from_file_location("check_encoding_v2", _MODULE_PATH)
assert _spec is not None and _spec.loader is not None
check_encoding = importlib.util.module_from_spec(_spec)
sys.modules["check_encoding_v2"] = check_encoding
...
def test_main_without_argument_uses_cwd(git_repo, monkeypatch, capsys):
    ...
    monkeypatch.setattr(sys, "argv", ["check_encoding_v2.py"])
...
def test_main_returns_one_when_git_fails(tmp_path, monkeypatch, capsys):
    outside = tmp_path / "kein_repo"
    outside.mkdir()
    monkeypatch.setattr(sys, "argv", ["check_encoding_v2.py", str(outside)])
