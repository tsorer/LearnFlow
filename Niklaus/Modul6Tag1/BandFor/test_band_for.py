...
report.md ist der Schoenwetterpfad. Weil das Modell die Datei nicht immer
schreibt, wird das Urteil des Evaluators zusaetzlich in die Konsole ausgegeben
(forward_subagent_text) — so ist das Ergebnis auch ohne Datei sichtbar.
...
opts = ClaudeAgentOptions(
    ...
    # Subagent-Text in den Stream forwarden, damit das Urteil des Evaluators auch
    # dann in der Konsole erscheint, wenn er report.md nicht schreibt.
...
async def main() -> int:
    ...
    # Ordnet Subagent-Nachrichten ihrer Rolle zu: die Agent-Delegation traegt eine
    # tool_use-id, und die Nachrichten des gestarteten Subagenten referenzieren sie
    # als parent_tool_use_id.
    delegation_ziel: dict[str, str] = {}
    evaluator_urteil: list[str] = []  # Konsolen-Fallback, falls report.md fehlt

    async for msg in query(prompt=f"Aufgabe: {AUFGABE}", options=opts):
        if isinstance(msg, AssistantMessage):
            herkunft = delegation_ziel.get(msg.parent_tool_use_id or "", "orchestrator")
            for block in msg.content:
                if isinstance(block, ToolUseBlock):
                    if block.name == "Agent":
                        # Sichtbar machen, WELCHE Definition gezogen wird, und die
                        # id merken, um spaeteren Subagent-Text zuzuordnen.
                        ziel = block.input.get("subagent_type")
                        delegation_ziel[block.id] = str(ziel)
                        print(f"\n>>> DELEGIERT an: {ziel!r}")
                    else:
                        print(f"\n>>> TOOL: {block.name}")
                elif isinstance(block, TextBlock):
                    if herkunft == "orchestrator":
                        print(block.text)
                        letzte_zeilen.append(block.text)
                    else:
                        # Subagent-Text (dank forward_subagent_text) — u.a. das
                        # Urteil des Evaluators, das wir als Fallback behalten.
                        print(f"[{herkunft}] {block.text}")
                        if herkunft == "evaluator":
                            evaluator_urteil.append(block.text)
    ...
    # ── Evaluator-Urteil in der Konsole (Fallback zu report.md) ──
    # report.md ist der Schoenwetterpfad; schreibt der Evaluator die Datei nicht,
    # ist sein Urteil dank forward_subagent_text trotzdem hier sichtbar.
    urteil = "\n".join(t.strip() for t in evaluator_urteil if t.strip())
    print("\n=== Evaluator-Urteil (Konsole) ===")
    print(urteil or "(kein Text vom Evaluator empfangen)")
    if not (CWD / ARTEFAKTE["evaluator"]).exists():
        print("(report.md wurde nicht geschrieben — Urteil kommt aus der Konsole)")
