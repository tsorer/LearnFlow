...
# `docker compose run`, nicht `docker exec src-api-1 ...`: genau diese
# Abweichung lässt den laufenden `api`-Container crash-loopen, `docker exec`
# griffe dann ins Leere (Ops/07_Pilotstart-Checkliste.md 2.2a).
docker compose run --rm api python apply_embedding_config.py
```
