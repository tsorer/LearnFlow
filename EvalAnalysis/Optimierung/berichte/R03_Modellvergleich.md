...
|---|---|---|---|---|---|
| `openai` | gpt-4o-mini | **90,9 %** | 22 | 2026-09-15T21-38-25Z | 997f08b |
| `qwen3-local` | ollama_chat/qwen3:8b | **59,1 %** | 22 | 2026-09-15T21-46-44Z | 997f08b |
| `gemma4-local` | ollama_chat/gemma4:26b | **81,8 %** | 22 | 2026-09-15T16-24-27Z | 0dedbaa |
| `gpt-oss-local` | ollama_chat/gpt-oss:20b | **90,9 %** | 22 | 2026-09-15T22-16-39Z | 997f08b |
| `ministral3-local` | ollama_chat/ministral-3:14b | **100,0 %** | 22 | 2026-09-15T22-49-37Z | 997f08b |
...
| `AIA-OOC-01` | Verweigerung | verweigert · Modell | verweigert · abgeschnitten | verweigert · Modell | verweigert · Modell | verweigert · Modell |
...
| `qwen3-local` | verweigert · Modell 12, ausgeliefert 9, verweigert · abgeschnitten 1 |
...
|---|---|---|---|---|---|---|---|
| `openai` | 24 | `stop` 24 | 0 | 0 | 18 s | 2,0 s | 0,6 s |
| `qwen3-local` | 24 | `stop` 23, `length` 1 | 0 | 0 | 904 s | 774,9 s | 3,9 s |
| `gemma4-local` | 25 | `stop` 25 | 0 | 0 | 1036 s | 93,3 s | 37,3 s |
| `gpt-oss-local` | 23 | `stop` 23 | 0 | 0 | 167 s | 20,7 s | 6,6 s |
| `ministral3-local` | 24 | `stop` 24 | 0 | 0 | 244 s | 49,2 s | 5,3 s |
...
|---|---|---|---|
| `openai` | 2450 | 2242 | 193 |
| `qwen3-local` | 3190 | 2753 | 8000 |
| `gemma4-local` | 2835 | 2267 | 1964 |
| `gpt-oss-local` | 2514 | 2285 | 196 |
| `ministral3-local` | 2920 | 2406 | 214 |
...
>
> Der Kontext deckt keine weiteren spezifischen Strafen ab.
...
> Schwerwiegende unerwünschte Ereignisse (SUSAR) müssen innerhalb von 7 Tagen oder innerhalb von 15 Tagen gemeldet werden, abhängig von der Art des Ereignisses. An lokale Ethikkommissionen sind nur lokale Ereignisse zu melden, während an die Leit-Ethikkommission alle Ereignisse bei Studienteilnehmern in der Schweiz gemeldet werden müssen [1].
...
> Der Grundbedarf für einen 1-Personen-Haushalt pro Monat wird nicht explizit genannt [3]. Die Pauschalbeträge für den Grundbedarf sind allgemein festgelegt, aber keine spezifische Angabe für einen 1-Personen-Haushalt ist vorhanden [3].
...
> Die Höhe der individuellen Prämienverbilligung (IPV) ist von Kanton zu Kanton verschieden [1]. Wie die IPV konkret beantragt wird, wird nicht im Kontext angegeben [1].
...
> Die Frage kann nicht beantwortet werden, da der Kontext keine Informationen über die Schweiz oder ihre nationalen Behörden enthält [1][2][3][4][5].
...
> Das deutsche KI-Umsetzungsgesetz sieht folgende Strafen für Verstöße vor:
> - Bei Verstößen gegen die einschlägigen Bestimmungen der Verordnung können Geldbußen von bis zu 3 % des gesamten weltweiten Jahresumsatzes im vorangegangenen Geschäftsjahr oder 15 000 000 EUR verhängt werden, je nachdem, welcher Betrag höher ist [2].
...
> Die ICH-GCP-Leitlinie stellt eine Reihe von Detailanforderungen an das Monitoring klinischer Studien, darunter die Normierung der Rechte und Pflichten der Prüfpersonen sowie der Sponsoren [1]. Sie verweist auf die Helsinki-Deklaration, ist aber deutlich detaillierter und umfassender als diese [1]. Zudem sind die Rechte und Pflichten der Prüfpersonen (d.h. der Forschenden) sowie der Sponsoren, die in finanzieller oder organisatorischer Hinsicht die Verantwortung für einen klinischen Versuch übernehmen, normiert [1]. Die ICH-Leitlinie hat zum Ziel, für die Durchführung klinischer Versuche mit Arzneimitteln international einheitliche Qualitätsstandards zu schaffen, um die gegenseitige Anerkennung klinischer Forschungsdaten zu erleichtern [1].
...
> Ergänzungsleistungen (EL) werden berechnet, indem bei der Anspruchsberechnung ausschließlich die gekürzte AHV‑Rente als Einnahme angerechnet wird. Alle anderen Einkünfte, die aus der AHV‑ oder IV‑Rente resultieren, werden nicht berücksichtigt, sodass keine Leistungskürzungen erfolgen und das soziale Existenzminimum im Alter gesichert bleibt. [2]
