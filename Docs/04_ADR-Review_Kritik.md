...
**→ ADR-007 für Chunking & Retrieval-Strategie fehlt noch.** *(ADR-006 wurde für den Background-Worker-Entscheid vergeben — siehe `04_ADR-006_Background-Worker.md`.)*

### Risiko 2 (signifikant): Celery + Redis als implizite Architektur-Entscheidung
~~Der Widerspruch zwischen ADR-001 und ADR-002 ist nie explizit entschieden.~~

**✅ Entschieden 2026-05-27:** pgqueuer (PostgreSQL-nativer Job-Queue) — kein Redis, kein Celery. Dokumentiert in `04_ADR-006_Background-Worker.md`.
...
|---|---|---|
| ADR-001 | ~~Celery + Redis widerspricht "ein Deployment-Artefakt"~~ → **✅ entschieden: pgqueuer (ADR-006)** | erledigt |
| ADR-002 | LangChain als Entscheidung fehlt; Celery+Redis-Mitigation → **✅ pgqueuer (ADR-006)** | offen: LangChain |
| ADR-003 | `bytea` vs. Large Objects ungelöst; ~~Azure-Referenz~~ → **✅ MinIO** | offen: bytea/lo |
| ADR-004 | Kostenschätzung fehlt; OpenAI Data Opt-Out nicht erwähnt; Rate Limiting fehlt | Mittel |
| ADR-005 | Chunking-Strategie fehlt → **ADR-007 ausstehend** | Kritisch |

## Offene nächste Schritte

1. **ADR-007 schreiben** — Chunking & Retrieval-Strategie (vor Tech Spike Woche 1)
2. **ADR-003 `bytea` vs. `lo` entscheiden** — bevor das Datenbankschema erstellt wird
3. **ADR-004** — Kostenschätzung und OpenAI Data Opt-Out ergänzen

---

*Quellen: Docs/04_ADR-001 bis 04_ADR-006 · Docs/02_Requirements.md · BFH CAS ADAI 2026*
