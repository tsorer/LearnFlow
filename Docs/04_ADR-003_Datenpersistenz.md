...
| **MinIO / S3-kompatibler Object Storage (für Originaldateien)** | Betrifft nur die Datei-Ablage, nicht die DB-Wahl: Originaldokumente in Object Storage statt als `bytea` würde WAL-Bloat und Backup-Last (siehe negative Konsequenzen) vermeiden — die sauberere Lösung bei vielen/grossen Dateien. Für den MVP bewusst zurückgestellt zugunsten der `bytea`-Variante (ein einziger Backup-/Restore-Pfad, kein zweiter Service), da bei < 500 Pilot-Dokumenten der WAL-/Grössen-Effekt beherrschbar bleibt. Erste Migrationsoption, sobald Datenmenge oder Redundanz-Anforderungen steigen. |
