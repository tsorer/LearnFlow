...
- **Sitzungsende serverseitig ausgelöst:** Der In-Memory-JWT kann während der Nutzung ablaufen (T-06: 1 h), ohne dass das Frontend es von sich aus merkt. Ein `401` auf eine Anfrage **mit** Token gilt deshalb als Sitzungsende: der API-Client verwirft den Auth-State, die Protected Route leitet auf `/login`, und der Grund wird dort ausgewiesen. Ein `401` von `/auth/login` selbst bleibt davon ausgenommen — er sendet keinen Token und bedeutet falsche Zugangsdaten (T-40).
