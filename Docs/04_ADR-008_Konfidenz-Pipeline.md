...
**Latenz gemessen (T-22, #29, 2026-09-06).** Stufe 3 addiert einen zweiten Provider-Aufruf auf eine Anfrage, auf die ohne Streaming (ADR-002) jemand wartet. Das Grenzband begrenzt die *Häufigkeit*, nicht die Dauer im Einzelfall. `make perf` (5 Konten parallel, 45 In-Corpus-Fragen des Gold-Datasets) misst **p95 = 4.53 s** (mean 2.44 s, min 0.84 s, max 5.09 s) — unter der Performance-NFA (p95 ≤ 10 s), mit deutlichem Puffer. Einzelwerte in `src/backend/perf/out/latency-results.csv`.
...
5. ~~**Latenz von Stufe 3 messen (T-22, #29):**~~ **erledigt (2026-09-06)** — `make perf`
   misst p95 = 4.53 s gegen die Performance-NFA (p95 ≤ 10 s), deutlich darunter. Sollte sich
